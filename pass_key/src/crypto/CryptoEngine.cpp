...
bool CryptoEngine::generateECDSAKeyPair()
{
    ...
    // 复制到成员变量（通过序列化/反序列化，mbedtls_pk_copy 在某些版本中不可用）
    if (ecdsaReady) {
        mbedtls_pk_free(&pkContext);
    }
    ...
    ret = mbedtls_pk_parse_key(&pkContext, derStart, derLen, NULL, 0,
                                mbedtls_ctr_drbg_random, &ctrDrbg);
    mbedtls_pk_free(&pk);
    if (ret != 0) {
        Serial.printf("[Crypto] ECDSA 私钥重新导入失败: ret=%d\n", ret);
        return false;
    }
    ecdsaReady = true;
